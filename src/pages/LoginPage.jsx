    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-#F5F5F5">
            <div className="w-full max-w-md">
                <h2 className="text-2xl font-bold text-center text-[#D9176C] mb-8">Welcome Back!</h2>
                <Formik initialValues={{ email: "", password: "", remember: false }} validationSchema={validationSchema} onSubmit={(values) => handleLogin(values)}>
                    <Form className="space-y-6">
                        {apiError && <div className="text-red-600 text-sm bg-red-100 p-3 rounded-lg">{apiError.includes("email") ? "Email is invalid or not registered" : apiError}</div>}
                        ...
                        <div className="flex items-center justify-between">
                            <label className="flex items-center cursor-pointer">
                                <Field type="checkbox" name="remember" className="w-4 h-4 text-[#D9176C] rounded" />
                                <span className="mr-2 text-sm text-gray-700">Remember me</span>
                            </label>
                            <Link to="/forgot-password" className="text-sm text-[#D9176C] hover:underline">
                                Forget password?
                            </Link>
                        </div>

                        {/* Button */}
                        <button type="submit" className="w-full bg-[#D9176C] hover:bg-[#B01258] text-white font-semibold py-3 rounded-lg transition-colors">
                            Log in
                        </button>
                    </Form>
                </Formik>
                <p className="text-center text-gray-600 mt-6">
                    ...
                    <Link to="/signup" className="text-[#D9176C] font-semibold hover:underline">
                        Signup
                    </Link>
                </p>
                {/* Divider */}{" "}
